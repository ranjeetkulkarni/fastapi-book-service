...
# FIX: Changed 'async def' to 'def' everywhere below
...
def create_book(book_data: BookCreateModel, session: Session = Depends(get_session)):
...
def update_book(book_uid: str, update_data: BookUpdateModel, session: Session = Depends(get_session)):
...
def delete_book(book_uid: str, session: Session = Depends(get_session)):
